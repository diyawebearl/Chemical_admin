...
//           item?.product?.name_of_chemical.toLowerCase().includes(productNameFilter.toLowerCase()) &&
//           (statusFilter === "" || item.status.toLowerCase() === statusFilter.toLowerCase()) && // Filter by status
//           (buyerFilter === "" || item.buyer_company.company_name.toLowerCase().includes(buyerFilter.toLowerCase())) && // Filter by buyer
//           (sellerFilter === "" || item.seller_company.company_name.toLowerCase().includes(sellerFilter.toLowerCase())) && // Filter by seller
//           (inquiryTypeFilter === "" || item.inq_type.toLowerCase().includes(inquiryTypeFilter.toLowerCase())) && // Filter by inquiry type
//           (stateFilter === "" || item.buyer_company.state.toLowerCase().includes(stateFilter.toLowerCase())) &&  // Filter by state
//           (cityFilter === "" || item.buyer_company.city.toLowerCase().includes(cityFilter.toLowerCase())) && // Filter by city
//           (!selectedDate || new Date(item.createdAt).toDateString() === selectedDate.toDateString()) // Filter by selected date or display all if selectedDate is null
//       )
//       .map((category) => ({
//         product: (
...
export default function AuthorsTableData({ categoryList, productNameFilter, statusFilter, buyerFilter, sellerFilter, inquiryTypeFilter, stateFilter, cityFilter, selectedDate }) {
  ...
  .filter(
    (item) =>
      ...
      (buyerFilter === "" || (item.buyer_company?.company_name || '').toLowerCase().includes(buyerFilter.toLowerCase())) &&
      (sellerFilter === "" || (item.seller_company?.company_name || '').toLowerCase().includes(sellerFilter.toLowerCase())) &&
      (inquiryTypeFilter === "" || item.inq_type.toLowerCase().includes(inquiryTypeFilter.toLowerCase())) &&
      (stateFilter === "" || (item.buyer_company?.state || '').toLowerCase().includes(stateFilter.toLowerCase())) &&
      (cityFilter === "" || (item.buyer_company?.city || '').toLowerCase().includes(cityFilter.toLowerCase())) &&
